...
use leptos_flow_renderer::{Renderer, RendererType, RenderStats};
...
impl WasmViewport {
    ...
    pub fn new(offset_x: f64, offset_y: f64, zoom: f64) -> Self {
        Self {
            viewport: Viewport::new(Position::new(offset_x, offset_y), zoom),
        }
    }
...
impl WasmNode {
    ...
    pub fn set_data(&mut self, data: JsValue) {
        *self.inner.data_mut() = data;
    }

    pub fn move_by(&mut self, dx: f64, dy: f64) {
        let current = self.inner.position();
        self.inner.set_position(Position::new(current.x + dx, current.y + dy));
    }
}
...
impl WasmEdge {
    ...
    pub fn new(id: &str, source_id: &str, target_id: &str, data: JsValue) -> Self {
        let edge = Edge::builder(EdgeId::new(id))
...
impl WasmGraph {
    ...
    pub fn get_node(&self, node_id: &str) -> Option<WasmNode> {
        let id = NodeId::new(node_id);
        self.inner.node(&id).map(|node| WasmNode {
            inner: node.clone(),
        })
    }
    ...
    pub fn get_edge(&self, edge_id: &str) -> Option<WasmEdge> {
        let id = EdgeId::new(edge_id);
        self.inner.edge(&id).map(|edge| WasmEdge {
            inner: edge.clone(),
        })
    }
...
impl WasmFlowEditor {
    ...
    pub fn render(&mut self) -> Result<WasmRenderStats, JsValue> {
        if let Some(renderer) = &mut self.renderer {
            ...
            let stats = renderer.render_graph(&self.graph.inner, &self.viewport.viewport)
                .map_err(|e| JsValue::from_str(&e.to_string()))?;
