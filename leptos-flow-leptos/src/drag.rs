...
use leptos_flow_core::{Graph, Position, NodeId, Rect, GroupId, GroupManager};
...
impl DragHandler {
    ...
    fn apply_bounds_constraints(&self, position: Position, node_size: &leptos_flow_core::Size) -> Position {
        ...
        E: Clone,
    {
        for node_id in &flow_state.selected_nodes {
            if let Some(node) = graph.get_node_mut(node_id) {
                ...
                node.set_size(leptos_flow_core::Size::new(new_width, new_height));
            }
        }
    }

    /// Check if two nodes overlap using axis-aligned bounding box collision detection
    fn nodes_overlap(&self, pos1: Position, size1: &leptos_flow_core::Size, pos2: Position, size2: &leptos_flow_core::Size) -> bool {
    ...
    fn calculate_target_position(&self, flow_state: &FlowState, node: &leptos_flow_core::Node<impl Clone>) -> Position {
...
mod tests {
    use super::*;
    use leptos_flow_core::{Node, Size, GroupManager, Group};
