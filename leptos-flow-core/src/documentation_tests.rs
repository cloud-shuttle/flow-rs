...
mod tests {
    ...
    fn test_core_documentation_files_exist() {
        ...
        for file in &required_files {
            ...
            assert!(path.exists(), "Required documentation file should exist: {}", file);
        }
    }
    ...
    fn test_example_documentation_exists() {
        ...
        let example_dirs = ["simple-flow", "leptos-demo"];
        for example in &example_dirs {
            let example_path = examples_dir.join(example);
            assert!(example_path.exists(), "Example directory should exist: {}", example);

            let readme_path = example_path.join("README.md");
            assert!(readme_path.exists(), "Example should have README.md: {}", example);
        }
    }
    ...
    fn test_api_documentation_completeness() {
        ...
        assert!(lib_rs.contains("//! # Leptos Flow Core"), "lib.rs should have proper module documentation");
        ...
        for module in &public_modules {
            assert!(lib_rs.contains(module), "Public module should be declared: {}", module);
        }
    }
    ...
    fn test_readme_accuracy() {
        ...
        assert!(readme.contains("tests-"), "README should contain test status badge");
        ...
        for feature in &current_features {
            assert!(readme.contains(feature), "README should mention feature: {}", feature);
        }
    }
    ...
    fn test_architecture_doc_accuracy() {
        ...
        for component in &current_components {
            assert!(arch_doc.contains(component), "Architecture doc should mention component: {}", component);
        }
    }
    ...
    fn test_api_design_accuracy() {
        ...
        for type_name in &current_types {
            assert!(api_doc.contains(type_name), "API design doc should mention type: {}", type_name);
        }
    }
    ...
    fn test_simple_flow_example_documentation() {
        let example_readme = fs::read_to_string("../../examples/simple-flow/README.md").unwrap();

        // Test that example has proper documentation
        assert!(example_readme.contains("# Simple Flow Example"), "Example should have proper title");
        assert!(example_readme.contains("## Features"), "Example should document features");
        assert!(example_readme.contains("## Usage"), "Example should document usage");
    }
    ...
    fn test_example_build_instructions() {
        ...
        assert!(example_readme.contains("trunk serve"), "Example should have build instructions");
        assert!(example_readme.contains("cargo"), "Example should mention cargo");
    }
    ...
    fn test_version_consistency() {
        ...
        assert!(!readme.contains("version = \"0.0.0\""), "README shouldn't have placeholder version");
    }
    ...
    fn test_documentation_style_consistency() {
        ...
        assert!(readme.starts_with("# "), "README should start with H1 title");
        ...
        for section in &required_sections {
            assert!(readme.contains(section), "README should have section: {}", section);
        }
    }
    ...
    fn test_code_examples_formatting() {
        ...
        assert!(readme.contains("```rust"), "README should have Rust code examples");
        assert!(readme.contains("```bash"), "README should have bash code examples");
    }
    ...
    fn test_internal_links_valid() {
        ...
        assert!(!readme.contains("](nonexistent.md)"), "README shouldn't have broken internal links");
    }
    ...
    fn test_all_public_apis_documented() {
        ...
        assert!(lib_rs.contains("// Re-export commonly used types"), "Should document re-exports");
        assert!(lib_rs.contains("/// Core prelude for convenient imports"), "Should document prelude");
    }
    ...
    fn test_error_documentation_completeness() {
        ...
        assert!(error_rs.contains("/// Main error type for flow operations"), "Should document main error type");
        ...
        for variant in &error_variants {
            assert!(error_rs.contains(&format!("/// {}", variant)), "Should document error variant: {}", variant);
        }
    }
    ...
    fn test_usage_examples_compilable() {
        ...
        if readme.contains("use leptos_flow_core::") {
            assert!(readme.contains("Graph"), "Should import Graph if using leptos_flow_core");
        }
    }
    ...
    fn test_performance_claims_documented() {
        ...
        if readme.contains("high-performance") {
            assert!(readme.contains("Performance"), "Should have performance section if claiming high performance");
        }

        if readme.contains("WASM") {
            assert!(readme.contains("WebAssembly"), "Should explain WASM if mentioned");
        }
    }
    ...
    fn test_testing_documentation_accuracy() {
        ...
        assert!(testing_doc.contains("Test Results"), "Testing doc should report test results");
        assert!(testing_doc.contains("passing"), "Testing doc should mention passing tests");
    }
    ...
    fn test_release_documentation_completeness() {
        ...
        assert!(release_doc.contains("Release Checklist"), "Should have release checklist");
        assert!(release_doc.contains("Pre-Release Phase"), "Should document pre-release process");
        assert!(release_doc.contains("Version Bump"), "Should document version bumping");
    }
}
